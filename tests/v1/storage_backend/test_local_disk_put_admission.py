...
def _create_worker() -> LocalDiskWorker:
    ...
    worker.put_tasks = []
    worker.submit_task = MagicMock(return_value=object())
    return worker
...
def _create_backend(
    *,
    ...
    backend.disk_lock = threading.RLock()
    backend.cache_policy = MagicMock()
    backend.batched_remove = MagicMock()
...
def test_rejected_put_can_be_retried_after_capacity_is_released() -> None:
    ...
    backend.cache_policy.get_evict_candidates.assert_not_called()
    backend.batched_remove.assert_not_called()
    ...
    backend.cache_policy.update_on_put.assert_called_once_with(key)
...
def test_insufficient_evictable_capacity_does_not_remove_resident_keys() -> None:
    ...
    backend.cache_policy.get_evict_candidates.assert_not_called()
    backend.batched_remove.assert_not_called()
    backend.cache_policy.update_on_put.assert_not_called()
    memory_obj.ref_count_up.assert_not_called()
    schedule.assert_not_called()
